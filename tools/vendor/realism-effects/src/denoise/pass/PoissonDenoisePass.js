...
export class PoissonDenoisePass extends Pass {
	...
	constructor(camera, textures, options = defaultPoissonBlurOptions) {
		...
		// Three.js 0.162+ removed WebGLMultipleRenderTargets; new API is
		// WebGLRenderTarget with `count` in options + `.textures[]` (plural).
	...
	get texture() {
		// Returns the textures array — callers index it as `pass.texture[0]`
		// for the diffuse channel and `pass.texture[1]` for specular.
	...
	render(renderer) {
		for (let i = 0; i < 2 * this.iterations; i++) {
			...
			this.fullscreenMaterial.uniforms["inputTexture2"].value =
				i === 0 ? this.textures[1] : inputRenderTarget.textures[1]
