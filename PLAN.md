...
- `pixl-engine export-pixlland <project> <out>` — bundle pronto pra upload no Pixlland. V0.1 pode ser thin wrapper que chama `export-three` + ZIPa o output. Útil mas não bloqueia "usar a engine" — o `<outDir>` do `export-three` já é portátil pra qualquer host (file://, GitHub Pages, itch.io, Pixlland).
